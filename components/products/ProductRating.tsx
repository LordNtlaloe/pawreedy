...
const ProductRatings = ({ isReadOnly, isEnabled, product }: { isReadOnly: boolean, isEnabled: boolean, product?: any }) => {
    ...
    const checkUserRatingEligibility = async () => {
      if (userId && product?._id) {
        if (product) {
          ...
          setUserCanRate(product > ratingCount);
        }
      }
    };
  ...
  const saveRating = async (formData: FormData) => {
    ...
    await sendMail({
      ...
      body: `<p>Dear ${userName},</p><p>Your rating for ${currentProduct?.productName} has been submitted successfully.</p><p>Thank you for your feedback!</p>`,
    });
  };
  ...
  return (
    <main className='p-2'>
      ...
      {showRatingComment && (
        ...
        >
          <h1>
            Rate {currentProduct?.productName} <span className='bg-primary/50 px-1 rounded'>: Your Rating: {selectedValue}</span>{' '}
          </h1>
