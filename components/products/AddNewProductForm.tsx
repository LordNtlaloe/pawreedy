...
const saveProduct = async (formData: FormData) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            // Wait for the image to be uploaded and URL received
            ...
            formData.append("imageUrl", imageUrl); // Appending the URL as a string
...
const handleImageChange = (e: any) => {
    ...
    if (image) {
        setImage(image); // Updated to handle File object
        setImagePreview(URL.createObjectURL(image) as any);
    } else {
...
const uploadImage = async () => {
    ...
    try {
        ...
        if (uploadedImageData?.secure_url) {
            return uploadedImageData.secure_url; // Ensure this is returned
        } else {
            throw new Error("Image upload failed");
        }
    } catch (error: any) {
        showConfirmationMessage("Error: ", `Error Uploading Image: ${error.message}`);
        setImage("");
        setImagePreview(null);
        setIsLoading(false);
        return null;
        ...
        <div className="space-y-12">
            <div className="p-2">
                <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div className="col-span-full">
                        <label htmlFor="name" className="block text-sm font-medium leading-6 text-gray-900">Quantity</label>
                        <div className="mt-2">
                            <div className="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                                <InputText type="text" name="quantity" id="quantity" placeholder="Product Quantity" required={false} className="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" text={''} />
                            </div>
                        </div>
                    </div>
                    <div className="col-span-full">
                            ...
                            <div className="text-center">
                                ...
                                <div className="mt-4 flex text-sm leading-6 text-gray-600">
                                    <label htmlFor="file_input" className="relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-600 focus-within:ring-offset-2 hover:text-indigo-500">
                                        ...
                                        <input aria-describedby="file_input_help" id="file_input" name="image" type="file" accept="image/png, image/jpg, image/jpeg" className="sr-only" onChange={handleImageChange} />
                                    </label>
                                    <p className="pl-1">or drag and drop</p>
                                </div>
                                <p className="text-xs leading-5 text-gray-600" id="file_input_help">PNG, JPG, GIF up to 10MB</p>
                            </div>
                        </div>
                        {iconPreview && (
